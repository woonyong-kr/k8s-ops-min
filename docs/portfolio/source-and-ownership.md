...
| 5 | 카탈로그 조회 router 를 앱에 연결하고 API 테스트 추가 | **미완료** — router 파일은 있으나 앱에 연결되지 않았고 API 테스트가 없음 |
| 6 | 판단과 수정 과정이 변경 이력에 남음 | 진행 중 |

5번이 남아 있으므로 조회 API는 **구현 완료가 아니라 작성 완료** 상태로 적습니다. README 의 "FastAPI 카탈로그 조회 엔드포인트 7종"도 같은 기준으로 읽어야 합니다.
