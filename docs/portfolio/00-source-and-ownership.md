...
이 코드는 프로젝트 종료 후 작업본에서 준비됐습니다. 원본 팀 프로젝트 이력에는 없으므로 개인 프로젝트 성과로 기재하지 않습니다.

다음 조건을 모두 통과한 뒤에만 후속 개인 프로젝트로 승격한다.

1. 정상·부분 실패·드리프트 시나리오를 로컬에서 직접 재현합니다.
2. Airflow DAG의 소스 중복 조회 여부를 확인하고 수정과 테스트를 남깁니다.
...
6. 실행자 본인의 변경 이력으로 판단과 수정 과정이 남습니다.
