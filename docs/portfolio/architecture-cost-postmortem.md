...
더 중요한 사실은 원본 배포의 **실제 event/s, payload p95, consumer lag 이력이 보존되지 않았다는 점**입니다. AWS 29.68TB를 이벤트 수로 나누는 것도 불가능합니다. 이 결손 때문에 현재 이력서에는 “초당 N건을 운영에서 처리했다”고 쓸 수 없습니다.
