...
저는 운영 데이터의 수집·정규화 계약을 맡았고, 이후 메타데이터 카탈로그·배치 파이프라인·정합성 검사·조회 API를 설계·구현해 등록한 스키마와 실제 데이터가 어긋나는지 검증하는 계층으로 확장하고 있습니다.

크래프톤 정글 12기 최종 프로젝트 · 5인 팀 · 2026.06.22–07.25
팀 전체 저장소는 `[팀 저장소 링크]`에 있습니다.
...
이 저장소는 팀 저장소의 사본입니다. 프론트엔드·GitOps 상태·배포 차트를 포함한 전체 코드가 그대로 들어 있고, 제 담당 범위는 위 그림과 아래 각 항목의 `개인`·`팀` 표시를 따릅니다.
