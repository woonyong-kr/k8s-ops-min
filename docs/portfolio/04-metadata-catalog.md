...
"이 자산에는 이런 필드가 있다"를 적어 두는 것만으로는 아무것도 못 잡는다. **등록 정보와 실제 데이터를 기계가 비교할 수 있어야** 의미가 있습니다.
...
**작동하지 않습니다.** 적재가 `ON CONFLICT DO UPDATE`라서 이전 세대 해시가 제자리에서 덮인다. 갱신이 끝나면 해시는 항상 하나다. 이 검사는 적재가 도중에 끊긴 경우에만 우연히 발화합니다.

그래서 `schema_observations`를 append-only로 분리했습니다. `(asset_id, schema_version, schema_hash)`가 유일 키다. 같은 계약이 반복 관측되면 행이 늘지 않고, **계약이 바뀌면 행이 하나 생깁니다.** 판정 기준점이 여기서 생깁니다.
...
`NO_DATA`와 `FAILED`를 합치면 재처리가 매일 전량을 다시 긁는다. 나누면 실패한 소스만 다시 돌린다.

실시간 계층의 3-state와 대응은 이렇다.
...
검사는 6종입니다. 각각 SQL 파일과 1:1로 대응합니다.
...
간선에 `run_id`를 저장하고, **조회할 때 `collection_runs`에 조인해 확인 시각을 함께 반환합니다.** 저장만 하고 조인하지 않으면 "이 관계가 언제 확인된 것인지"에 답할 수 없습니다. 7일이 지난 간선은 `STALE_EDGE`로 잡는다.

`raw_snapshots.s3_uri`를 통해 정규화 행 → 실행 → 원본 snapshot까지 역추적하도록 모델링했습니다. 다만 현재 값은 실제 S3 URI가 아니라 `file://...` 로컬 파일 경로입니다.

**단, `content_hash`만으로 snapshot 행까지 중복 제거하면 이 경로가 끊깁니다.** 이틀 연속 내용이 같아도 `raw_snapshots`는 `(run_id, content_hash)`를 키로 사용해 실행별 행을 남깁니다. 현재 로컬 파일은 날짜·source별로 저장하며, 객체 저장소 수준의 내용 중복 제거는 구현하지 않았습니다.
...
- 정규화 결과에서 로컬 원본 snapshot까지 역추적 경로를 모델링했다
