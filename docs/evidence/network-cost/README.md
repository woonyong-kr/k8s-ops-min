...
이 디렉터리는 `서비스를 작게 나누면 책임과 실패를 격리하기 쉽다`는 초기 판단이 실제 AWS 실행 환경에서 어떤 비용을 만들었는지 검증하기 위한 증거 묶음입니다. 이미지에 적힌 숫자는 아래 CSV와 코드에서 다시 계산할 수 있습니다.
...
AWS 청구서는 개인 식별자, 계정 ID, 인스턴스 ID와 Auto Scaling Group 랜덤 suffix를 제거했습니다. 숫자는 지우지 않았습니다.
...
- 통합 controller는 코드 조립 검증까지 완료했지만, 같은 부하로 AWS에 재배포해 비용 감소를 관측하지 않았습니다. 그래서 아직 “비용을 몇 % 절감했다”고 쓰지 않습니다.
