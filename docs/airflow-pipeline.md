...
원천 상태는 이 넷이 전부입니다. 한때 여기에 다섯 번째로 `NO_SOURCE_DATA`("과거 원본이 없어 재생 불가")를 적어 뒀는데, 코드의 상태 어휘에는 없는 값이었습니다. 뺐습니다.

조회 응답에는 상태와 별개로 **사유 코드**가 붙습니다. 상태를 어휘로 늘리면 성공률 집계처럼 상태를 세는 모든 곳이 같이 바뀌어야 하지만, 사유는 "왜 그랬나"를 덧붙일 뿐이라 세는 방식을 건드리지 않습니다.
...
**둘을 나눈 것이 중요합니다.** 조회 API 가 한때 이 둘을 한 질의로 뽑아 전부 `SOURCE_FAILED` 로 내보냈습니다. 잘림을 실패로 합치지 않겠다는 것이 이 프로젝트의 출발점인데, 정작 응답에서 다시 합치고 있었습니다. 지금은 상태를 사유로 옮기는 표(`COLLECTION_STATUS_REASONS`)를 통해서만 변환하고, 그 표를 테스트로 고정했습니다(`test_원천_결과가_사유_코드로_그대로_옮겨진다`).
...
다음이 끝나기 전에는 이걸 Airflow 운영 경험이라고 부르지 않습니다.
