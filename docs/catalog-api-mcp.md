...
팀 프로젝트에서 AI 조회 도구 계층을 만들었지만 최종 경로에서 빠졌습니다. 코드는 저장소에 남아 있습니다. 실사용자가 없었기 때문입니다. 그 판단은 [범위 판단](scope-decisions.md)에 있습니다.

거기서 세운 규칙은 두 가지였습니다. **누가 실제로 쓰는가. 잘못됐을 때 알아차릴 방법이 있는가.**

카탈로그 MCP를 그 규칙에 정직하게 대보면 이렇습니다.

| 기준 | 상태 |
|---|---|
| 실사용자 | **아직 없습니다.** 카탈로그를 뒤지는 사람은 상정한 것이지 관측한 것이 아닙니다 |
| 잘못됐을 때 알아차릴 방법 | **부분적.** 도구 호출은 감사 로그에 남지만, 자연어 질의가 엉뚱한 도구로 갔는지 판정할 평가셋이 없습니다 |

두 기준을 완전히 만족하지 못한 채로 만들었습니다. 만든 이유는 **위험이 이전과 다르다**고 봤기 때문입니다.

이전 도구 계층은 새 조회 능력을 만들려 했습니다. 잘못되면 없던 경로가 생깁니다.
카탈로그 MCP는 이미 있는 읽기 전용 API를 노출만 합니다. 이 서버만 놓고 보면 잘못돼도 **잘못된 답이 나올 뿐 이 서버가 무엇을 바꾸지는 않습니다.** 다만 "아무것도 망가지지 않는다"고까지는 쓸 수 없습니다 — 같은 세션에 쓰기 도구가 붙어 있으면 여기서 나온 잘못된 답이 그쪽 입력이 됩니다. 그 경로는 [아래](#반환-데이터가-신뢰할-수-없는-입력이다)에서 따로 다룹니다.
...
| `get_resource_state` | `/v1/catalog/resources/state` | 이 리소스 지금 믿어도 되나 |
...
flowchart LR
    ...
    MCP -->|"RFC 8693 token exchange"| STS["인증 서버<br/>Security Token Service"]
...
**길이와 문자를 제한합니다.** 제어 문자와 개행을 이스케이프하고 필드당 길이를 자릅니다. 길이 제한만으로는 주입을 막을 수 없지만 페이로드 크기를 줄입니다.

**결과가 도구 선택을 바꾸지 못하게 합니다.** 도구는 일곱 개 읽기 전용뿐이고 목록이 고정입니다. 반환값이 새 도구를 등장시키지 않습니다.

**그럼에도 완전히 막지는 못합니다.** 같은 세션에 다른 서버의 쓰기 도구가 붙어 있으면, 카탈로그가 읽기 전용인 것과 무관하게 주입된 지시가 그쪽으로 갈 수 있습니다. 이건 이 서버가 혼자 풀 수 있는 문제가 아니라 **에이전트 호스트의 세션 정책 문제**입니다. 그렇게 적어 둡니다.

### 분류는 권한이 아니다

`data_assets.classification`은 조회 필터로 쓰입니다. **접근 제어 입력으로는 쓰지 않습니다.**

즉 `search_assets(classification="sensitive")`로 어떤 자산이 민감으로 표시됐는지 열거할 수 있습니다. 내용은 못 보지만 **목록은 보입니다.**

지금 이 상태인 이유는 카탈로그의 모든 자산이 같은 팀 소유이고 전 사용자가 동일 권한이기 때문입니다. 소유자가 나뉘면 분류를 인가 입력으로 승격해야 합니다. **분류가 필터로만 쓰이는 상태에서 "권한을 넘지 않는다"고 쓰면 과장입니다.**
...
- 자연어 질의 정확도 평가셋이 없습니다. 어떤 질문이 어떤 도구로 가야 맞는지 판정할 기준이 없습니다
- 감사 로그를 남기지만 분석 화면이 없습니다
- 세션에 다른 서버의 쓰기 도구가 함께 붙은 경우는 이 서버가 통제할 수 없습니다
- `classification`이 인가 입력이 아닙니다. 자산 목록은 분류별로 열거됩니다
- 카탈로그 외 도메인 API는 MCP로 노출하지 않았습니다
