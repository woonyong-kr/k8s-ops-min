...
이 프로젝트에서 Data Foundation에 가장 가까운 일은 데이터를 많이 모은 것이 아닙니다. **소비자가 불완전한 데이터를 완전한 데이터로 오해하지 않도록 수집 범위와 실패 상태를 계약으로 만든 것**입니다.
...
네 소스는 형식과 시간 개념이 다릅니다. Prometheus는 시계열 수치, Loki는 stream과 log line, Tempo는 trace와 span, Kubernetes API는 현재 리소스 상태를 반환합니다. 이를 같은 JSON 모양으로 억지로 평탄화한 것이 아니라, provider별 payload를 유지하면서 모든 결과에 공통 수집 맥락과 신뢰 범위를 붙였다.

이 판단이 필요한 이유는 소비자 때문입니다. `partial`을 모르면 RCA는 일부 근거만으로 결론을 내리고, inventory 저장소는 잘린 목록을 실제 삭제로 해석할 수 있습니다.
...
| Git 협업·리뷰 | 리뷰 지적 39분 뒤 경계 조건 보강 커밋 | [엔지니어링 로그](../docs/engineering-log.md#4-리뷰-40분-뒤) | 팀 |
| 진행 공유·문서화 | 계약 변경과 문서를 같은 커밋에서 갱신 | [엔지니어링 로그](../docs/engineering-log.md) | 팀 |
...
| AWS·클라우드 | EKS 배포와 담당 수집 경로 설정, 청구 원장 분석 | [비용 회고](../docs/architecture-cost-postmortem.md) | 팀 |
...
팀 프로젝트에서 다룬 메타데이터는 개별 evidence를 설명합니다.
...
정식 데이터 카탈로그는 조직의 데이터 자산을 관리합니다.
...
따라서 현재 팀 프로젝트 경험을 “데이터 카탈로그 구축”으로 바꾸어 부르지 않습니다. **운영 evidence metadata 계약을 구현했으며, 그 경험을 카탈로그의 schema·lineage·quality 영역으로 확장하고 있다**고 말합니다.

## Airflow를 기존 실시간 수집기와 바꾸지 않는 이유

기존 프로젝트에는 이미 실시간 장애 분석용 스케줄러와 agent poll 경로가 있습니다. Airflow로 이를 교체하면 같은 실행 책임이 두 곳에 생기고, 짧은 주기의 DAG 이력·권한·watermark·중복 구간을 새로 해결해야 합니다.

Airflow의 자연스러운 역할은 다음 후속 작업입니다.
...
이건 실시간 장애 수집을 대체하지 않습니다. 이미 수집된 데이터를 일정에 따라 재검사하고, 실패 task만 재시도하고, 과거 구간을 backfill하는 역할입니다. 그래서 적용 자체는 억지스럽지 않다.
...
이 문장은 문제, 판단, 구현, 검증이 모두 원본 코드와 Git 이력으로 연결됩니다.

## 후속 확장을 이력서에 올리는 완료 조건

1. PostgreSQL과 Airflow를 직접 기동합니다.
2. 정상·한 소스 실패·전 소스 실패·downstream 실패를 각각 재현합니다.
