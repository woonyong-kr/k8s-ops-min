...
**위쪽은 장애가 나면 도는 실시간 경로**입니다. 네 원천는 응답 형식도 시간 기준도 실패하는 방식도 다릅니다. 하나로 모으면서 각 데이터를 어디까지 믿어도 되는지 함께 넘기는 것이 제 일이었습니다. 원인 판정은 제가 준 데이터로 돌아가기 때문에, 절반만 왔다는 사실을 알리지 않으면 판정도 절반짜리 근거로 결론을 냅니다.
...
- 카탈로그 계층 테스트 100종 (MCP 신뢰 경계 17 · 응답 경계 12 · 프로토콜 8 · 조회 API 15 · 결과 적재 키 13 · 문서·코드 대조 18 · 그 외 17)

→ [검사 SQL 열 개](docs/sql-quality-checks.md) · [측정과 한계](docs/load-and-design-limits.md) · 확인 `make catalog-sql` `make catalog-bench`

### 조회 API 및 MCP

- FastAPI 카탈로그 조회 엔드포인트 8종 작성 / 커서 페이지네이션 / 모든 응답에 마지막 실행 상태 부착 (앱 배선 `app.py` · API 테스트 15종 포함)
...
- MCP 인자 스키마 검증 및 신뢰할 수 없는 입력 표시 (경계 테스트 10종)
...
make catalog-test      # 카탈로그 테스트   → 100 passed
...
- 관측 470만 행 / 3.4GB 까지 측정했습니다. 그 지점에서 검사 8종 합계 2,590ms 이고 그중 80% 를 중복 적재 검사 하나가 씁니다. 정확성을 위해 전체 스캔을 감수한 결과이며, 2단계 질의로 1.76배 줄어드는 것까지 확인했으나 아직 적용하지 않았습니다. 수억 행 규모는 외삽할 수 없습니다 — [측정과 한계](docs/load-and-design-limits.md)
